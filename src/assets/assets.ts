...
export const getMotivationalMessage = (caloriesConsumed: number, activeMinutes: number, DAILY_CALORIE_LIMIT: number) => {
    ...
    if (caloriesConsumed === 0 && activeMinutes === 0) {
        return { text: "Ready to crush today? Start registering!", emoji: "💪" };
    }
    if (percentage > 100) {
        return { text: "Over the limit, but tomorrow is a new day!", emoji: "🌅" };
    }
    if (percentage >= 80) {
        return { text: "Amost at your limit, stay tuned!", emoji: "⚡" };
    }
    if (activeMinutes >= 30) {
        return { text: "Great workout today! Keep.", emoji: "🔥" };
    }
    if (percentage >= 50) {
        return { text: "You're daoing great, Keep going!", emoji: "✨" };
    }
    return { text: "Every step counts.You got it!", emoji: "🚀" };
};
